...
for (aa in seq(n_cl)) {
  ...
  for (ii in seq(3)) {
    ...
    Cases.fit[[aa]][[ii]]<-glmmTMB(cases ~ 
                                     ...
                                     scale(data_fit$AGE_GR2)+scale(data_fit$AGE_GR4)+scale(data_fit$MALE)+scale(POVERTY)+
    ...
    Cases.fit.uni[[aa]][[5]][[ii]]<-glmmTMB(cases ~ scale(data_fit$POVERTY)+(1|State)
                                            + offset(log(POP)), data = data_fit,family = nbinom2, ziformula  = ~ 1
    )
    Cases.fit.uni[[aa]][[6]][[ii]]<-glmmTMB(cases ~ scale(data_fit$POP_DENSITY)+(1|State)
                                            + offset(log(POP)), data = data_fit,family = nbinom2, ziformula  = ~ 1
    )
    print(paste(aa,ii))
  }
...
for (aa in seq(n_cl)) {
  ...
  for (ii in seq(3)) {
    ...
    deaths.fit.uni[[aa]][[5]][[ii]]<-glmmTMB(deaths ~ scale(data_fit$POVERTY)+(1|State)
                                            + offset(log(POP)), data = data_fit,family = nbinom2, ziformula  = ~ 1
    )
    deaths.fit.uni[[aa]][[6]][[ii]]<-glmmTMB(deaths ~ scale(data_fit$POP_DENSITY)+(1|State)
                                            + offset(log(POP)), data = data_fit,family = nbinom2, ziformula  = ~ 1
    )
    print(paste(aa,ii))
  }
...
for (aa in seq(n_cl)) {
  ...
  for (ii in seq(3)) {
    ...
    deaths.fit[[aa]][[ii]]<-glmmTMB(deaths ~ 
                                     scale(data_fit$ATKINSON)+
                                     scale(data_fit$AGE_GR2)+scale(data_fit$AGE_GR4)+scale(data_fit$MALE)+scale(data_fit$POVERTY)+
                                     scale(data_fit$POP_DENSITY)+
                                     (1|State)
                                   + offset(log(POP)), data = data_fit,family = nbinom2, ziformula  = ~ 1
    )
...
jpeg("C:/Users/hongh/Dropbox (Yale_FES)/COVID19/USCOVID/Race_COVID/Manuscript/Figures/FigureS3(POVERTY_ADJUSTED).jpg",
...
save.image("C:\\Users\\hongh\\Dropbox (Yale_FES)\\COVID19\\USCOVID\\Race_COVID\\Manuscript\\Rimages\\Cases_Deaths_RR_POVERTY_ADJUSTED.Rdata")
